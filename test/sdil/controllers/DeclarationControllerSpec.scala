  ...
  "Declaration controller" should {
    ...
    "submit a valid Subscription to the backend on POST if all required form pages are complete" in {
      ...
      val expected = Subscription(
        ...
        Seq(PackagingSite(UkAddress(List("1 Production Site St", "Production Site Town"), "AA11 1AA"), None, None, None)),
        Seq(WarehouseSite(UkAddress(List("1 Warehouse Site St", "Warehouse Site Town"), "AA11 1AA"), None, None, None)),
        Contact(
          Some("A person"),
          Some("A position"),
          "1234",
