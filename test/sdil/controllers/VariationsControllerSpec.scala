...
class VariationsControllerSpec extends ControllerSpec {
  ...
  "VariationsController" should {
    ...
    "When a user has enrolled with a subscirption " in {
      val sdilEnrolment = EnrolmentIdentifier("EtmpRegistrationNumber", "XZSDIL000100107")
      when(mockAuthConnector.authorise[Enrolments](any(), matching(allEnrolments))(any(), any())).thenReturn {
        Future.successful(Enrolments(Set(Enrolment("HMRC-OBTDS-ORG", Seq(sdilEnrolment), "Active"))))
      }
    ...
    "closedSites" in {
      val result = controller.closedSites(sites, closedSites)
      result mustEqual (any)
    }

    "closedWarehouseSites" in {
      val result = controller.closedWarehouseSites(registeredUserInformation)
      result mustEqual (any)
    }

    "closedPackagingSites" in {
      val result = controller.closedPackagingSites(registeredUserInformation)
      result mustEqual (any)
    }

    "newPackagingSites" in {
      val result = controller.newPackagingSites(registeredUserInformation)
      result mustEqual (any)
    }

    "newWarehouseSites" in {
      val result = controller.newWarehouseSites(registeredUserInformation)
      result mustEqual (any)
    }
    ...
    "show variations complete" in {
      ...
      when(mockRegVariationsCache.get(anyString())(any())).thenReturn(Future.successful(None))
      ...
      when(mockSdilConnector.balanceHistory("sdilRef", true)(any())).thenReturn {
    ...
    "change actor status" in {
      ...
      val result = controller.changeActorStatus("").apply(FakeRequest())
      status(result) mustEqual OK
    }
  }
}
