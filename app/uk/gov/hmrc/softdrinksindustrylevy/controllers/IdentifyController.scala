...
import uk.gov.hmrc.softdrinksindustrylevy.config.FormDataCache
import uk.gov.hmrc.softdrinksindustrylevy.models.Identification
  ...
  def apply(): Form[Identification] = {
    Form(
      mapping(
        ...
        "postcode" -> text.verifying("error.postcode.invalid", _.matches(postcodeRegex))
