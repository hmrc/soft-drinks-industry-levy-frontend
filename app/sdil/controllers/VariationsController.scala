...
class VariationsController(
  ...
  private def contactUpdate(
    ...
    def askContactChangeType: WebMonad[Set[ContactChangeType]] =
      if (data.isVoluntary) {
        askSet(
          "contact-change-type",
          Set(ContactChangeType.ContactPerson,
              ContactChangeType.ContactAddress),
          minSize = 1,
          None,
    ...
    for {
      ...
      packSites <- if (change.contains(Sites)) {
        manyT(
          "packSites",
          ask(packagingSiteMapping, _)(packagingSiteForm,
                                       implicitly,
                                       implicitly,
                                       implicitly),
        ...
        ) emptyUnless (data.producer.isLarge
          .contains(true) || data.copackForOthers)
      } else data.updatedProductionSites.pure[WebMonad]

      warehouses <- if (change.contains(Sites)) {
        manyT(
          "warehouses",
          ask(warehouseSiteMapping, _)(warehouseSiteForm,
                                       implicitly,
                                       implicitly,
                                       implicitly),
      ...
      businessAddress <- if (change.contains(ContactAddress)) {
        ask(addressMapping,
            "businessAddress",
            default = data.updatedBusinessAddress)
  ...
  private def activityUpdate(
    ...
    for {
      ...
      useCopacker <- ask(bool(), "useCopacker", data.usesCopacker) when packLarge
        .contains(false)
      ...
      copacks <- ask(litres, "copackQty") emptyUnless ask(bool(),
                                                          "copacker",
                                                          data.copackForOthers)
      imports <- ask(litres, "importQty") emptyUnless ask(bool(),
                                                          "importer",
                                                          data.imports)
      noUkActivity = (copacks, imports).isEmpty
      smallProducerWithNoCopacker = packLarge.forall(_ == false) && useCopacker
        .forall(_ == false)
      ...
      variation <- if (shouldDereg)
        tell(
          "suggestDereg",
          uniform
            .confirmOrGoBackTo("suggestDereg", "packLarge")) >> deregisterUpdate(
          data)
      else {
        ...
        for {
          usePPOBAddress <- ask(bool(), "pack-at-business-address")(
            implicitly,
            implicitly,
            extraMessages,
            implicitly) when packer && data.original.productionSites.isEmpty
          ...
          firstPackingSite <- ask(packagingSiteMapping,
                                  "first-production-site")(packagingSiteForm,
                                                           implicitly,
                                                           ExtraMessages(),
                                                           implicitly) when
            pSites.isEmpty && packer
          packSites <- askPackSites(
            pSites ++ firstPackingSite
              .fold(List.empty[Site])(x => List(x))) emptyUnless packer

          isVoluntary = packLarge.contains(false) && useCopacker.contains(true) && (copacks,
                                                                                    imports).isEmpty
  ...
  private def deregisterUpdate(
    ...
    for {
      reason <- askBigText(
        ...
        errorOnEmpty = "error.reason.empty")
      deregDate <- ask(
        startDate
          .verifying("error.deregDate.nopast", _ >= LocalDate.now)
          .verifying("error.deregDate.nofuture",
  ...
  private def fileReturnsBeforeDereg[A](returnPeriods: List[ReturnPeriod],
                                        data: RegistrationVariationData) =
    for {
      sendToReturns <- tell("file-return-before-deregistration",
                            uniform.fragments.return_before_dereg(
  ...
  private def program(
    ...
    for {
      variableReturns <- execute(
        sdilConnector.returns.variable(base.original.utr))
      returnPeriods <- execute(sdilConnector.returns.pending(subscription.utr))
      onlyReturns = subscription.deregDate.nonEmpty
      changeTypes = ChangeType.values.toList.filter(x =>
        variableReturns.nonEmpty || x != ChangeType.Returns)
      changeType <- askOneOf(
        "changeType",
        changeTypes,
        helpText = Html(Messages("change.latency")).some) when !onlyReturns
      variation <- changeType match {
        ...
        case Some(ChangeType.Deregister) if returnPeriods.isEmpty =>
          deregisterUpdate(base)
        case Some(ChangeType.Deregister) if returnPeriods.nonEmpty =>
  ...
  private def chooseReturn[A](
    ...
    for {
      ...
      messages = variableReturns.map { x =>
        s"returnPeriod.option.${x.year}${x.quarter}" -> s"${Messages(
          s"returnPeriod.option.${x.quarter}")} ${x.year}"
      }.toMap ++ Map(
        "error.radio-form.choose-option" -> "error.radio-form.choose-option.returnPeriod")

      extraMessages = ExtraMessages(messages)

      returnPeriod <- askOneOf(
        "returnPeriod",
        variableReturns.sortWith(_ > _).map(x => s"${x.year}${x.quarter}"))(
        extraMessages)
        .map(
          y =>
            variableReturns
              .filter(x =>
                x.quarter === y.takeRight(1).toInt && x.year === y.init.toInt)
              .head)
      _ <- clear
      _ <- resultToWebMonad[A](
        Redirect(
          routes.VariationsController.adjustment(
            year = returnPeriod.year,
            quarter = returnPeriod.quarter,
            id = "check-your-variation-answers")))
  ...
  private def adjust(
    ...
    for {
      ...
      origReturn <- execute(
        connector.returns.get(base.original.utr, returnPeriod))
        .map(_.getOrElse(throw new NotFoundException(
          s"No return for ${returnPeriod.year} quarter ${returnPeriod.quarter}")))

      newReturn <- askReturn(base.original,
                             sdilRef,
                             sdilConnector,
                             returnPeriod,
                             origReturn.some)

      variation = ReturnVariationData(origReturn,
                                      newReturn,
                                      returnPeriod,
...
//TODO Brought forward should not be 0
      ...
      extraMessages = ExtraMessages(
        messages = Map(
          "heading.check-your-variation-answers" -> s"${Messages(
            s"returnPeriod.option.${variation.period.quarter}")} ${variation.period.year} return details",
          "return-variation-reason.label" -> s"Reason for correcting ${Messages(
            s"returnPeriod.option.${variation.period.quarter}")} return"
        ))

      isSmallProd <- execute(
        isSmallProducer(sdilRef, sdilConnector, returnPeriod))

      _ <- checkYourReturnAnswers(
        "check-your-variation-answers",
        variation.revised,
        broughtForward,
        base.original,
        isSmallProd,
      ...
      reason <- askBigText(
        ...
        constraints =
          List(("error.return-variation-reason.tooLong", _.length <= 255)),
        errorOnEmpty = "error.return-variation-reason.empty"
      )(extraMessages)
      repayment <- askOneOf("repayment", List("credit", "bankPayment"))(
        ltbs.play.scaffold.SdilComponents.extraMessages) when variation.revised.total - variation.original.total < 0
      _ <- checkReturnChanges(
        "check-return-differences",
        variation.copy(reason = reason, repaymentMethod = repayment))
      _ <- execute(
        sdilConnector.returns.vary(
          sdilRef,
          variation.copy(reason = reason, repaymentMethod = repayment)))
      _ <- clear
      exit <- journeyEnd("returnVariationDone",
                         whatHappensNext = uniform.fragments
                           .variationsWHN(key = Some("return"))
                           .some)(extraMessages)

    } yield exit
  }

  def checkYourRegAnswers(key: String,
  ...
  def closedWarehouseSites(variation: RegistrationVariationData): List[Site] = {
    closedSites(variation.original.warehouseSites,
                Convert(variation).closeSites.map(x => x.siteReference))
  }

  def closedPackagingSites(variation: RegistrationVariationData): List[Site] = {
    closedSites(variation.original.productionSites,
                Convert(variation).closeSites.map(x => x.siteReference))
  }

  def newPackagingSites(variation: RegistrationVariationData): List[Site] = {
    variation.updatedProductionSites
      .diff(variation.original.productionSites)
      .toList
  }

  def newWarehouseSites(variation: RegistrationVariationData): List[Site] = {
    variation.updatedWarehouseSites
      .diff(variation.original.warehouseSites)
      .toList
  }

  private def closedSites(sites: List[Site],
                          closedSites: List[String]): List[Site] = {
    sites
      .filter { x =>
  ...
  def index(id: String): Action[AnyContent] = registeredAction.async {
    implicit request =>
      val sdilRef = request.sdilEnrolment.value
      val persistence = SaveForLaterPersistence("variations", sdilRef, cache)
      sdilConnector.retrieveSubscription(sdilRef) flatMap {
        case Some(s) =>
          runInner(request)(program(s, sdilRef))(id)(persistence.dataGet,
                                                     persistence.dataPut,
                                                     JourneyConfig(SingleStep))
  ...
  private def changeBusinessAddressTemplate(
    ...
    val unitMapping: Mapping[Unit] = Forms.of[Unit](new Formatter[Unit] {
      override def bind(
          key: String,
          data: Map[String, String]): Either[Seq[FormError], Unit] = Right(())

      override def unbind(key: String, value: Unit): Map[String, String] =
        Map.empty
    })
  ...
  def changeBusinessAddress(id: String): Action[AnyContent] =
    registeredAction.async { implicit request =>
      val sdilRef = request.sdilEnrolment.value
      val persistence = SaveForLaterPersistence("variations", sdilRef, cache)
      sdilConnector.retrieveSubscription(sdilRef) flatMap {
        case Some(s) =>
          runInner(request)(changeBusinessAddressJourney(s, sdilRef))(id)(
            persistence.dataGet,
            persistence.dataPut,
            JourneyConfig(SingleStep))
        case None => NotFound("").pure[Future]
  ...
  private def changeBusinessAddressJourney(
    ...
    for {
      _ <- changeBusinessAddressTemplate("change-registered-account-details",
                                         subscription)
      ...
      exit <- journeyEnd("variationDone",
                         whatHappensNext =
                           uniform.fragments.variationsWHN().some)
    } yield exit
  }

  def changeActorStatus(id: String): Action[AnyContent] =
    registeredAction.async { implicit request =>
      val sdilRef = request.sdilEnrolment.value
      val persistence = SaveForLaterPersistence("variations", sdilRef, cache)
      sdilConnector.retrieveSubscription(sdilRef) flatMap {
        case Some(s) =>
          runInner(request)(changeActorStatusJourney(s, sdilRef))(id)(
            persistence.dataGet,
            persistence.dataPut,
            JourneyConfig(SingleStep))
        case None => NotFound("").pure[Future]
  ...
  def adjustment(year: Int, quarter: Int, id: String): Action[AnyContent] =
    registeredAction.async { implicit request =>
      val sdilRef = request.sdilEnrolment.value
      val persistence =
        SaveForLaterPersistence(s"adjustments-$year$quarter", sdilRef, cache)
      sdilConnector.retrieveSubscription(sdilRef) flatMap {
        case Some(s) =>
          runInner(request)(
            adjust(s, sdilRef, sdilConnector, ReturnPeriod(year, quarter)))(id)(
            persistence.dataGet,
            persistence.dataPut,
            JourneyConfig(LeapAhead))
        case None => NotFound("").pure[Future]
      }
    }
}
