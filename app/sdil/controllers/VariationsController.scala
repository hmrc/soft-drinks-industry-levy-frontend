...
class VariationsController(
  ...
  private def contactUpdate(
    ...
    val extraMessages = ExtraMessages(
      messages = Map("heading.contact-details" -> Messages("heading.contact"))
    )
    ...
    for {
      ...
      packSites <- if (change.contains(Sites)) {
                    manyT(
                      ...
                      ask(packagingSiteMapping, _)(packagingSiteForm, implicitly, extraMessages, implicitly),
      ...
      warehouses <- if (change.contains(Sites)) {
                     manyT(
                       "warehouse-details",
                       ask(warehouseSiteMapping, _)(warehouseSiteForm, implicitly, extraMessages, implicitly),
      ...
      contact <- if (change.contains(ContactPerson)) {
                  ask(contactDetailsMapping, "contact-details", data.updatedContactDetails)(
                    implicitly,
                    implicitly,
                    extraMessages,
                    implicitly)
      ...
      businessAddress <- if (change.contains(ContactAddress)) {
                          ask(addressMapping, "business-address", default = data.updatedBusinessAddress)(
                            implicitly,
                            implicitly,
                            extraMessages,
                            implicitly)
  ...
  private def activityUpdate(
    ...
    for {
      ...
      useCopacker <- ask(bool("third-party-packagers"), "third-party-packagers", data.usesCopacker) when packLarge
                      .contains(false)
      packageOwn <- askOption(litreagePair.nonEmpty, "packaging-site")(
                     approxLitreageForm,
                     implicitly,
                     implicitly,
                     implicitly) when packLarge.nonEmpty
      copacks <- askOption(litreagePair.nonEmpty, "contract-packing")(
                  approxLitreageForm,
                  implicitly,
                  implicitly,
                  implicitly)
      ...
      variation <- if (shouldDereg && (returnPeriods.isEmpty || isVoluntary))
                    tell(
                      ...
                      uniform.confirmOrGoBackTo("suggest-deregistration", "amount-produced")) >> deregisterUpdate(data)
  ...
  private def deregisterUpdate(
    ...
    val extraMessages = ExtraMessages(
      messages = Map(
        "heading.cancel-registration-reason.orgName" -> s"${data.original.orgName}",
    ...
    for {
      ...
      deregDate <- ask(
                    cancelRegDate
                      .verifying("error.cancel-registration-date.nopast", _ >= LocalDate.now)
                      .verifying("error.cancel-registration-date.nofuture", _ < LocalDate.now.plusDays(15)),
                    "cancel-registration-date"
                  )(implicitly, implicitly, extraMessages, implicitly)
    } yield
      data.copy(
        reason = reason.some,
  ...
  private[controllers] def programInner(
    ...
    for {
      ...
      extraMessages = ExtraMessages(
        messages = if (variation.deregDate.nonEmpty) {
          Map(
      ...
      exit <- journeyEnd(id = "variationDone", subheading = subheading, whatHappensNext = whn.some)(extraMessages)
    } yield exit
  }
  ...
  private def chooseReturn[A](
    ...
    for {
      ...
      returnPeriod <- askOneOf("select-return", variableReturns.sortWith(_ > _).map(x => s"${x.year}${x.quarter}"))(
                       extraMessages)
                       .map(
                         y =>
                           variableReturns
                             .filter(x => x.quarter === y.takeRight(1).toInt && x.year === y.init.toInt)
                             .head)
      _ <- clear
      _ <- resultToWebMonad[A](
            Redirect(
              routes.VariationsController
                .adjustment(year = returnPeriod.year, quarter = returnPeriod.quarter, id = "return-details")))
  ...
  private def adjust(
    ...
    for {
      ...
      reason <- askBigText(
                 ...
                 constraints = List(("error.return-correction-reason.tooLong", _.length <= 255)),
                 errorOnEmpty = "error.return-correction-reason.empty"
               )(extraMessages)
      ...
      exit <- journeyEnd(
               ...
               whatHappensNext = uniform.fragments
                 .variationsWHN(
                   ...
                   key = Some("return"),
                   broughtForward = broughtForward.some)
                 .some
             )(extraMessages)
  ...
  def adjustment(year: Int, quarter: Int, id: String): Action[AnyContent] = registeredAction.async { implicit request =>
    ...
    sdilConnector.retrieveSubscription(sdilRef) flatMap {
      case Some(s) =>
        runInner(request)(adjust(s, sdilRef, sdilConnector, ReturnPeriod(year, quarter)))(id)(
          persistence.dataGet,
          persistence.dataPut,
