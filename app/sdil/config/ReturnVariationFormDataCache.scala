...
class ReturnVariationFormDataCache @Inject()(
  ...
  def get(internalId: String)(implicit hc: HeaderCarrier): Future[Option[ReturnVariationData]] =
    fetchAndGetEntry[ReturnVariationData](s"$internalId-sdil-return-variation", "retVarFormData").recover {
      case _: JsResultException => None
    }

  def clear(internalId: String)(implicit hc: HeaderCarrier): Future[Unit] =
    remove(s"$internalId-sdil-return-variation") map { _ =>
      ()
    }
